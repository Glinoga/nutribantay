...
class ChildController extends Controller
{
    ...
    public function export(Request $request)
    {
        ...
        $exportType = $request->type === 'overaged' ? 'overaged children' : 'children';

        AuditLog::logAction([
            'action' => 'exported',
            'model_type' => 'Child',
            'description' => "{$exportType} exported by {$user->name}",
            'barangay' => $user->barangay,
        ]);
    ...
    public function exportCSV($children, ?string $exporterName = null)
    {
        ...
        $callback = function () use ($children, $exporterName) {
            ...
            fputcsv($file, ['Generated By', $exporterName ?? 'Unknown']);
            fputcsv($file, []);
            ...
            try {
                DB::transaction(function () use ($row, $user, $sex, $weight, $height, $hasAnthropometricData, &$imported, &$withoutHealthLog) {
                    ...
                    if ($hasAnthropometricData) {
                        ...
                        if (! empty($row['date_measured'])) {
                            $healthLogData['created_at'] = $row['date_measured'];
                        }

                        HealthLog::create($healthLogData);
    ...
    public function print(Request $request)
    {
        ...
        AuditLog::logAction([
            'action' => 'printed',
            'model_type' => 'Child',
            'description' => ($request->type === 'overaged' ? 'Overaged children' : 'Children list')." printed by {$user->name}",
            'barangay' => $user->barangay,
        ]);
    ...
    public function showPrint(Child $child)
    {
        ...
        AuditLog::logAction([
            ...
            'model_type' => 'Child',
            'model_id' => $child->id,
            'model_name' => $child->fullname,
            'description' => "Profile of {$child->fullname} printed by {$user->name}",
            'barangay' => $user->barangay,
        ]);
    ...
    public function exportSingle(Child $child)
    {
        ...
        AuditLog::logAction([
            ...
            'model_id' => $child->id,
            'model_name' => $child->fullname,
            'description' => "Profile of {$child->fullname} exported by {$user->name}",
            'barangay' => $user->barangay,
        ]);
        ...
        $callback = function () use ($child, $childVaccines, $childVitamins, $user) {
            $file = fopen('php://output', 'w');
            ...
            // Metadata rows
            fputcsv($file, ['Generated', now()->format('Y-m-d H:i:s')]);
            fputcsv($file, ['Generated By', $user->name]);
            fputcsv($file, []);
