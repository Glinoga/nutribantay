...
export default function Index({ healthlogs }: Props) {
    ...
    const isHealthworker = (auth?.roles ?? []).some((role) => role.toLowerCase() === 'healthworker');
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="m-4 mb-4 flex items-center justify-between">
                ...
                {canManageLogs && (
                    <Link href="/healthlogs/create" className="rounded bg-blue-600 px-4 py-2 text-white hover:bg-blue-700">
                        Add Health Log
                    </Link>
                )}
            </div>
            ...
            <form
                onSubmit={(e) => {
                    ...
                    router.get('/healthlogs', { search: formData.get('search') }, { replace: true });
                }}
            ...
            >
                <input
                    type="text"
                    name="search"
                    placeholder="Search by child name, sex, or barangay..."
                    ...
                    <tbody>
                        {healthlogs.map((log) => (
                            <tr key={log.id} className="hover:bg-gray-100">
                                ...
                                {/* CHILD NAME */}
                                <td className="border px-4 py-2">{log.child?.fullname ?? `Child #${log.child_id}`}</td>
                                ...
                                <td className="border px-4 py-2 font-semibold">{log.nutrition_status ?? '-'}</td>
                                ...
                                {/* CHECKBOX FIELDS */}
                                <td className="border px-4 py-2 text-center">{log.vitamin_a ? '✔️' : '❌'}</td>
                                <td className="border px-4 py-2 text-center">{log.deworming ? '✔️' : '❌'}</td>
                                ...
                                <td className="border px-4 py-2 whitespace-nowrap">
                                    <Link href={`/healthlogs/${log.id}`} className="mr-2 rounded bg-blue-500 px-3 py-1 text-white hover:bg-blue-600">
                                        View
                                    </Link>
