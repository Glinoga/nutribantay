...
import { Calendar, Clock, Edit2, Pill, Plus, Search, Sparkles, Trash2, Users, X } from 'lucide-react';
...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Vitamins', href: route('vitamins.index') }];

export default function Index({ vitamins, stats }: IndexProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingVitamin) {
            put(route('vitamins.update', { vitamin: editingVitamin.id }), {
                onSuccess: () => {
                    setShowModal(false);
    ...
    const openEdit = (vitamin: Vitamin) => {
        setEditingVitamin(vitamin);
        setData('name', vitamin.name);
        setData('description', vitamin.description ?? '');
        setShowModal(true);
    };
    ...
    const handleDelete = (vitamin: Vitamin) => {
        MySwal.fire({
            ...swalTheme(),
            title: `Remove "${vitamin.name}"?`,
            html: `
            ...
            if (result.isConfirmed) {
                router.delete(route('vitamins.destroy', { vitamin: vitamin.id }));
            }
        });
    };
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
                ...
                <div className="relative mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
                    ...
                    <div className="mb-6 pt-8 text-center">
                        <div className="mb-4 inline-flex items-center gap-3 rounded-full border border-teal-100/50 bg-white/90 px-6 py-3 shadow-lg backdrop-blur-sm dark:border-teal-800/50 dark:bg-gray-800/90">
                            ...
                            <span className="text-sm font-semibold text-teal-700 dark:text-teal-400">Vitamin Catalog</span>
                        </div>
                    </div>
                            ...
                            <div className="flex flex-col items-center justify-center gap-4 sm:flex-row">
                                ...
                                <div>
                                    ...
                                    <p className="mt-1 flex items-center justify-center gap-2 text-gray-600 dark:text-gray-300">
                                        ...
                                        Manage available vitamins for child supplementation tracking
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-4">
                        <div className="stat-card cursor-pointer rounded-xl border border-teal-100/50 bg-white p-4 shadow-md transition-all hover:border-teal-200 hover:shadow-lg dark:border-teal-800/50 dark:bg-gray-800/80">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Total Vitamins</p>
                        ...
                        <div className="stat-card cursor-pointer rounded-xl border border-blue-100/50 bg-white p-4 shadow-md transition-all hover:border-blue-200 hover:shadow-lg dark:border-blue-800/50 dark:bg-gray-800/80">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Children Supplemented</p>
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    {stats.most_popular_count > 0 && (
                                        <p className="text-xs text-gray-500 dark:text-gray-400">{stats.most_popular_count} children</p>
                                    )}
                                </div>
                                <div className="rounded-full bg-purple-50 p-2.5 dark:bg-purple-900/30">
                                    <Pill className="h-5 w-5 text-purple-500 dark:text-purple-400" />
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div className="relative max-w-md flex-1">
                            ...
                            <input
                                type="text"
                                placeholder="Search vitamins by name or description..."
                                value={search}
                    ...
                    {filteredVitamins.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-20 text-center">
                            <div className="mb-4 rounded-full bg-gray-100 p-4 dark:bg-gray-700">
                                <Pill className="h-12 w-12 text-gray-400 dark:text-gray-500" />
                            </div>
                            <p className="text-lg font-medium text-gray-900 dark:text-gray-100">
                                {search ? 'No vitamins match your search.' : 'No vitamins yet'}
                            </p>
                            <p className="text-sm text-gray-500 dark:text-gray-400">
                                {search ? 'Try adjusting your search terms.' : 'Add a vitamin using the button above to get started.'}
                            </p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                            {filteredVitamins.map((vitamin, index) => (
                                <div key={vitamin.id} className="vitamin-card group" style={{ animationDelay: `${index * 50}ms` }}>
                                        ...
                                        <div className="p-4">
                                            <div className="mb-3 flex items-start gap-3">
                                                ...
                                                <div className="min-w-0 flex-1">
                                                    <p className="truncate text-sm font-bold text-gray-900 dark:text-gray-100">{vitamin.name}</p>
                                                    <p className="text-xs text-gray-500 dark:text-gray-400">ID: {vitamin.id}</p>
                                                </div>
                                            </div>

                                            {vitamin.description && (
                                                <p className="mb-3 line-clamp-2 text-xs text-gray-600 dark:text-gray-300">{vitamin.description}</p>
                                            )}

                                            <div className="mb-3 flex items-center gap-2">
                                                <Badge className="bg-teal-50 text-teal-700 hover:bg-teal-100 dark:bg-teal-900/30 dark:text-teal-400">
                                                    <Users className="mr-1 h-3 w-3" />
                                                    {vitamin.children_count} {vitamin.children_count === 1 ? 'child' : 'children'}
                                                </Badge>
                                            </div>
                                            ...
                                            <div className="space-y-1.5 text-xs text-gray-500 dark:text-gray-400">
                                                <div className="flex items-center gap-1.5">
                                                    ...
                                                    <span className="truncate">{vitamin.created_by ?? 'Unknown'}</span>
                                                </div>
                                            </div>
                                        </div>
            ...
            <Dialog open={showModal} onOpenChange={(open) => !open && closeModal()}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle className="text-xl font-bold">{editingVitamin ? 'Edit Vitamin' : 'Add Vitamin'}</DialogTitle>
                        <DialogDescription>
                            {editingVitamin ? 'Update the vitamin details below.' : 'Add a new vitamin to the catalog.'}
                        </DialogDescription>
                    </DialogHeader>

                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="space-y-2">
                            <Label htmlFor="vitamin-name" className="dark:text-gray-300">
                                Vitamin Name
                            </Label>
                            <Input
                                id="vitamin-name"
                                ...
                                placeholder="e.g., Vitamin A, Iron, MNP"
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="vitamin-description" className="dark:text-gray-300">
                                Description (optional)
                            </Label>
                            <Textarea
                                id="vitamin-description"
                                value={data.description}
                                onChange={(e) => setData('description', e.target.value)}
                                placeholder="Optional description of the vitamin..."
                                rows={3}
                        ...
                        <DialogFooter>
                            ...
                            >
                                {processing ? 'Saving...' : editingVitamin ? 'Update' : 'Add Vitamin'}
                            </Button>
